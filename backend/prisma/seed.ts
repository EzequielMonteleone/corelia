...
async function main() {
  ...
  const rolesData = [
    {
      name: 'Admin',
      description: 'El administrador del edificio. Tiene control total sobre el edificio y sus usuarios.',
    },
    {
      name: 'Owner',
  ...
  const ownerPermissionKeys = ['UNIT_READ', 'EXPENSE_READ', 'PAYMENT_CREATE', 'PAYMENT_READ']
  ...
  const roomerPermissionKeys = ['EXPENSE_READ', 'PAYMENT_CREATE', 'PAYMENT_READ']
  ...
  const allRolePermissions = [...adminPermissions, ...ownerPermissions, ...roomerPermissions]
